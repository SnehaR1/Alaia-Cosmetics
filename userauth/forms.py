...
from adminapp.models import Address, ProductReview, ReturnedProduct
...
class OTPForm(forms.Form):
    otp = forms.CharField(
        max_length=6, widget=forms.TextInput(attrs={"placeholder": "Enter your OTP"})
    )
...
class CheckoutForm(forms.Form):
    ...
    address = forms.ModelChoiceField(
        queryset=Address.objects.none(), empty_label=None, widget=forms.RadioSelect
    )
...
class ReturnRequestForm(forms.ModelForm):
    reason_for_return = forms.CharField(required=True)
    order_item_id = forms.IntegerField()

    class Meta:
        model = ReturnedProduct
        fields = ["reason_for_return"]
